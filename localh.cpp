...
using namespace std;
	...
	GradingBo :: GradingBo (const double * ax1, const double * ax2)
	{
		h2 = 0.5 * (ax2[0] - ax1[0]);
		for (int i = 0; i < 3; i++)
	...
	LocalH :: LocalH (const Poi<3> & pmin, const Poi<3> & pmax, double agrading)
	{
		double x1[3], x2[3];
		double hmax;

		boundingbox = Bo<3> (pmin, pmax);
		grading = agrading;
		...
		root = new GradingBo (x1, x2);
		boxes.push_back (root);
	}


	LocalH :: LocalH (const Bo<3> & box, double agrading)
	{
		Poi<3> pmin = box.PMin();
		Poi<3> pmax = box.PMax();
	...
	//在p点设置局部尺寸
	void LocalH :: SetH (const Poi<3> & p, double h)
	{
		/*
	...
	//获取点的局部尺寸，通过八叉树二分查找
	double LocalH :: GetH (const Poi<3> & x) const
	{
		const GradingBo * box = root;
	...
	double LocalH :: GetMinH (const Poi<3> & pmin, const Poi<3> & pmax) const
	{ 
		Poi<3> pmin2, pmax2;
		for (int j = 0; j < 3; j++)
			if (pmin[j] < pmax[j])
	...
	double LocalH :: GetMinHRec (const Poi<3> & pmin, const Poi<3> & pmax,
		const GradingBo * box) const
	{
		double h2 = box->h2;
	...
	{
		ost << "LocalH: " << boxes.size() << " boxes of " << sizeof(GradingBo)
			<< " bytes = " << boxes.size()*sizeof(GradingBo) << " bytes" << endl;
	}
